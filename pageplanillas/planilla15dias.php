...
class PDF extends FPDF
{
	...
	function header()
	{
	    ...
	    $this->Cell(110);
	    // Título
	    $this->Cell(50,10,utf8_decode('Planilla salarial 15 días laborados'),0,0,'C');
	    // Salto de línea
	    $this->Ln(10);
	    ...
	    $this->Cell(20, 6, 'Hrs extras', 1,0, 'C', 0);
	    $this->Cell(20, 6, 'Hrs nocts.', 1,0, 'C', 0);
...
foreach ($empl as $empls) {
		...
		$horasextra = $empls[3] * $empls[4] * $valores[7][2]; //salarioxhora * horasextra * valordehorasextra
		$horasnoct = $empls[5] * $empls[3] * $valores[4][2]; //horasextra * salarioxhora * valordehorasnocturnas
		$gravado = number_format((float)($salario + $horasextra + $horasnoct), 2, '.', ''); //salario+horasextra+horasnoct
	    ...
	    $pdf->Cell(20, 6, number_format((float)($horasextra), 2, '.', ''), 1,0, 'C', 0);
	    $pdf->Cell(20, 6, number_format((float)($horasnoct), 2, '.', ''), 1,0, 'C', 0);
...
try{
    ...
    $result = $consulta->execute(
        array(':codT' => $codnueva,
        	':montoT' => $ttlrecib,
            ':detalles'=> 'Pago de planilla 15 días',
