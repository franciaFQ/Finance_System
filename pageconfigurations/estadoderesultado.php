...
class PDF extends FPDF
{
	function header()
	{
	    ...
	    $this->Cell(50);
	    // Título
	    $this->Cell(100,8,utf8_decode('ESTADO DE RESULTADO'),1,1,'C');
	    // Movernos a la derecha
	    // Times bold 18
	    ...
	    // Movernos a la derecha
	    $this->Cell(20);
	    $this->Cell(14, 6, '', 1,0, 'C', 0);
	    $this->Cell(100, 6, 'Cuenta', 1,0, 'C', 0);
...
$param .= '%';
$db = new Database();
$query = $db->connect()->query("select c.* from cuentas c where 
c.codCuenta in (41, 51, 6216, 45, 42) or c.codCuenta like '61%' or c.codCuenta like '62%' and c.codCuenta != 624");
...
foreach ($grupos as $grupo) {
		...
		//Movernos
		if ($grupo[1]== 42 || $grupo[1]== 42 || $grupo[1]== 42 || $grupo[1]== 42 || $grupo[1]== 42 || ) {
			$pdf->Cell(20);
		...
		where d.codCuenta = $grupo[0] and t.fechatrans like '$param' group by d.codCuenta");
